...
| `test_lebenslauf_abdeckung.py` | 13 | **§G21:** Jeder Ausstiegsgrund landet im Lebenslauf — `stop_intraday` war der einzige mit **0 %** Abdeckung, und weil er bei Einbrüchen feuert, fehlten dem Lernbericht die Verlusttrades. Der Wächter fragt „fehlt ein Eintrag, der **neuer** ist als der jüngste vorhandene?“ — sonst leuchtete er für die nicht nachtragbare Altlast dauerhaft ROT. Der Intraday-Pfad wird mit `dry_run=False` durchgespielt; nur so erreicht ein Test den Schreibzweig. |
